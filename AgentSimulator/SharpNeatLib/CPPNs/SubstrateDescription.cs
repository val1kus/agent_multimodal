...
namespace SharpNeatLib.CPPNs
{
    public class SubstrateDescription
    {
        ...
        public NeatGenome.NeatGenome generateMultiGenomeStack(INetwork network, uint numberOfAgents, bool normalizeWeights, bool adaptiveNetwork, bool modNetwork, out List<float> coords, bool evolvedSubstrate)
        {
            ...
            // Schrum: Hack to fix a hack: Original code always started z = -1, but this will not always be desired
            float coord = numberOfAgents == 1 ? 0 : -1.0f;
        ...
        public List<NeatGenome.NeatGenome> generateGenomeStackSituationalPolicy(INetwork network, uint numberOfAgents, bool normalizeWeights, bool adaptiveNetwork, bool modNetwork, int numBrains, out List<float> coords, bool forcedSituationalPolicyGeometry)
        {
            ...
            //Schrum: Signal scheme below is general, but may not make sense, because it imbeds policies in geometry
            for (int j = 0; j < numBrains; j++)
            {
                ...
                else if (numBrains % 2 == 0) // Schrum: for even modes, split into negative and positive
                {
                    ...
                    signal = (j - half) + (j < half ? 0 : 1);
                }
                else // Schrum: Must be odd: Make one be 0, then assign the rest to pos and neg
                {
                    int half = numBrains / 2; // Rounded down
                    signal = (j - half);
                }
                // Schrum: Original scheme removed (mine is more general, and subsumes this)
                //else
        ...
        public NeatGenome.NeatGenome generateGenomeStackSituationalPolicy(INetwork network, uint numberOfAgents, bool normalizeWeights, bool adaptiveNetwork, bool modNetwork, out List<float> coords, float signal)
        {
            ...
            return this.generateGenomeStackSituationalPolicy(network, coords, normalizeWeights, adaptiveNetwork, modNetwork, signal);
        }
